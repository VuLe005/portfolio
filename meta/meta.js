...
const row = (d) => ({
  ...
  // dates
  // CSV has columns like: date=YYYY-MM-DD, timezone=±HH:MM, datetime=ISO string
...
function renderCommitInfo(data, commits) {
  ...
  const [longestFile, maxLen] = d3.greatest(fileLengths, (d) => d[1]) ?? ['–', 0];
  dl.append('dt').text('Maximum file length (in lines)');
  dl.append('dd').text(maxLen);
...
// helper to update visuals based on a cut-off datetime
function updateVisualsUpTo(date) {
  filteredCommits = commits.filter((d) => d.datetime <= date);
  updateScatterPlot(data, filteredCommits);
  updateFileDisplay(filteredCommits);
}

// Step 3.3: Scrollama to update the scatter plot as you scroll
function onStepEnter(response) {
  const date = response.element.__data__.datetime;
  updateVisualsUpTo(date);
}
